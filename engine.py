...
class VideoEngine:
    ...
    def create_video_segment(self, img_path, audio_path, out_path, target_dur):
        ...
        # 核心修复：强制输出为 mpegts (.ts)，杜绝 MP4 分段拼接时的 DTS 时间戳错乱与合并缓慢问题
        cmd = [
    ...
    def build_video(self):
        self.load_config()
        self.emit("log", "Starting video build pipeline (TS Segment Mode)...")
        ...
        for i in range(max_items):
            idx = i + 1
            self.emit("progress", {"pct": int((idx/max_items)*50), "msg": f"正在生成素材 (项目 {idx}/{max_items})..."})
            ...
            for rep in range(1, repeat + 1):
                # 强制使用 .ts 后缀
                seg_out = os.path.join(self.temp_dir, f"item{idx}_rep{rep}.ts")
        ...
        self.emit("progress", {"pct": 80, "msg": "正在极速拼接所有 TS 片段..."})
        ...
        # 核心修复：使用 aac_adtstoasc 滤镜，完美将 TS 音频流转换为 MP4，解决播放器时钟漂移
        subprocess.run([
            self.ffmpeg, "-y", "-f", "concat", "-safe", "0", "-i", list_path, 
            "-c", "copy", "-bsf:a", "aac_adtstoasc", "-movflags", "+faststart", final_out
        ])
        
        self.emit("progress", {"pct": 100, "msg": "完成!"})
